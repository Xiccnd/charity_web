  ...
  <div id="navigationBar" class="js-nav">
    <ul class="mid">
      <li v-for="(item,index) in tags">
        <a href="javascript:void(0)" :class="checkindex(index)" @click="changcolor(index)">{{item}}</a>
      </li>
    </ul>
  </div>
</template>
...
export default {
  data(){
    return{
      ...
      type:0,
      path:'/default_index',
      paths:["/default_index","/volunteer_program","/volunteer_team","/volunteer_center","/team","/program","/media","/volunteer_program_details","/volunteer_team_details"]
    }
  },
  methods:{
  changcolor(index) {
