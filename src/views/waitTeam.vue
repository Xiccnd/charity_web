        ...
        <div id="updatedata">
          <li class="col v-t white tab-container">
            ...
            <div class="tabbar tabbar-tight mb-20 row w">
                <span data-a="1" class="col v-m" v-on:click="myTeam">我的队伍</span>
                <span data-a="2" class="col v-m active">待定队伍</span>
            ...
            <table class="user-table" style="text-align: center">
              ...
              <tr v-for="item in allTeam" style="height: 50px;">
                ...
                <td>
                  <a v-if="item.statusName === '退出审核' || item.statusName === '加入审核'" v-on:click="cancelApply(item.teamId, item.mark)">取消申请</a>
                </td>
              </tr>
            </table>
          </li>
...
export default {
  ...
  methods: {
    cancelApply: function (teamId, mark) {
      this.$http({
    ...
    getTeam: function () {
      this.$http({
        url: "/volunteersTeamid/selectMyJoinInTeam",
        method: "POST",
        data: {
          name: localStorage.getItem("username")
        }
